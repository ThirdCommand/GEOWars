...
import { BoxBox, type RotationState} from "./boxbox";
...
export class BoxBoxSprite extends LineSprite implements Spawnable{
    ...
    draw(ctx: CanvasRenderingContext2D) {
        ...
        const boxWidth = 11 * spawningScale; // only used for spawning?
    ...
    drawSpawningBoxBox(ctx: CanvasRenderingContext2D, w: number) {
        const r = 210;
        const g = 75;
        const b = 75;
        const blurFactor = 0.5;
    ...
    drawSpawningBox1(ctx: CanvasRenderingContext2D, w: number) {
        ctx.beginPath();
        ctx.moveTo(-w / 4, w / 4);
    ...
    drawSpawningBox2(ctx: CanvasRenderingContext2D, w: number) {
        ctx.beginPath();
        ctx.moveTo(w  / 4, -w  / 4);
        ctx.lineTo(w  / 4, (3 * w)  / 4);
        ctx.lineTo((-3 * w ) / 4, (3 * w ) / 4);
    ...
    drawBox1(ctx: CanvasRenderingContext2D) {
