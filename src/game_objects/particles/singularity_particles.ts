...
export class SingularityParticles extends GameObject {
    ...
    frequencyParticleCreation: number;
    particleCreationTime: number;
    currentColor: Color;
    particleNum: number;
    constructor(engine: GameEngine | AnimationView, transform: Transform) {
        super(engine);
    ...
    addSingularityParticle(){
        ...
        const pos: [number, number] = [r * Math.cos(theta) + this.transform.pos[0], r * Math.sin(theta) + this.transform.pos[1]];
        const vel: [number, number] = [speed * Math.cos(alpha) + this.transform.vel[0], speed * Math.sin(alpha) + this.transform.vel[1]];
