...
export class MissileExhaust extends GameObject {
    colorRange: number;
    ...
    positionOnMissile: [number, number];
    width: number;
    currentTime: number;
    exhaustRate: number;
    ...
    constructor(engine: GameEngine | AnimationView, transform: Transform, position: [number, number], width: number) {
        ...
        this.positionOnMissile = position;
        this.width = width;
        this.currentTime = 0;
    ...
    addMissileExhaustParticle() {
        ...
        // ]
        const exhaustPositionAngle = Math.atan2(this.positionOnMissile[1], this.positionOnMissile[0] + linePosition) - Math.PI / 2;
        const exhaustPositionLength = Math.sqrt(this.positionOnMissile[0]**2 +  (this.positionOnMissile[1] + linePosition)**2);
        const position: [number, number] = [
