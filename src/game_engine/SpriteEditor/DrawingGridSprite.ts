...
export class DrawingGridSprite extends LineSprite {
    ...
    drawXLines(ctx: CanvasRenderingContext2D) {
        ...
        GameScript.DIM_Y;
        const xLineCount = 36;
        const xLineIncrement = GameScript.DIM_Y / xLineCount;
        for(let i=0;i<xLineCount;i++) {
            ...
            if(i%3 === 0) {
                ...
                ctx.strokeStyle = "#808080";
                ctx.setLineDash([1, (GameScript.DIM_X - 121) / 120]);
            }
            ctx.moveTo(0,0 + i * xLineIncrement);
            ctx.lineTo(GameScript.DIM_X,0 + i * xLineIncrement);
            ctx.stroke();
        }
    }
    ...
    drawYLines(ctx: CanvasRenderingContext2D) {
        ...
        GameScript.DIM_Y;
        const yLineCount = 20;
        const yLineIncrement = GameScript.DIM_X / yLineCount;
        for(let i=0;i<yLineCount;i++) {
            if(i ===10) {
                ctx.strokeStyle = "#FFFFFF";
                ctx.beginPath();
                ctx.setLineDash([1, (GameScript.DIM_Y - 73) / 72]);
                ctx.moveTo(0 + i * yLineIncrement,0);
                ctx.lineTo(0 + i * yLineIncrement,GameScript.DIM_X,);
                ctx.stroke();
            } else {
                ctx.strokeStyle = "#808080";
                ctx.beginPath();
